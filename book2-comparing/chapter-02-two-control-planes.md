...
而且这不是停留在概念层面的命名好看。`fragment.rs` 里真的把 `AGENTS_MD_START_MARKER`、`AGENTS_MD_END_MARKER`、`SKILL_OPEN_TAG`、`SKILL_CLOSE_TAG` 定成常量，再由 `ContextualUserFragmentDefinition::wrap()` 和 `into_message()` 把内容包装成 `ResponseItem::Message`。到了 `user_instructions.rs`，`UserInstructions` 还会把目录名序列化进 `# AGENTS.md instructions for ...` 这一行；`SkillInstructions` 则额外带上 `<name>` 和 `<path>`。也就是说，Codex 连“这段规则来自哪个目录、哪个 skill 文件”都尽量不让模型自己猜。
